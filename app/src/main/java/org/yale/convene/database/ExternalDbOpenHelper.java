...
import org.yale.convene.BeenClass.Activitylist;
import org.yale.convene.BeenClass.Levelid;
...
import org.yale.convene.BeenClass.ResponsesData;
...
import org.yale.convene.BeenClass.facilities.FacilitiesList;
import org.yale.convene.BeenClass.facilities.Facility;
import org.yale.convene.BeenClass.facilitiesBeen.FacilitySubTypeBeen;
import org.yale.convene.BeenClass.parentChild.Level1;
...
import org.yale.convene.BeenClass.parentChild.LinkagesList;
...
import org.yale.convene.BeenClass.service.ServiceList;
...
public class ExternalDbOpenHelper extends SQLiteOpenHelper {
     ...
     * @return
     */
    public List<org.yale.convene.BeenClass.facilities.Datum> getFacilityDetailsFromDB(String btype, int filterOrder, String lastModifiedDate) {
        List<org.yale.convene.BeenClass.facilities.Datum> facilityList = new ArrayList<>();
        ...
        try {
            ...
            if (cursor.getCount() != 0 && cursor.moveToFirst()) {
                do {
                    org.yale.convene.BeenClass.facilities.Datum datum = new org.yale.convene.BeenClass.facilities.Datum();
    ...
    private void setServicesToBean(String service, org.yale.convene.BeenClass.facilities.Datum datum, List<Integer> servicesList) {
        String servicesSplitedString = service.replaceAll("\\[", "").replaceAll("\\]", "");
        if (!servicesSplitedString.isEmpty()) {
            String[] serviceList = servicesSplitedString.split(",");
            servicesList = new ArrayList<>();
     ...
     * @return
     */
    public List<org.yale.convene.BeenClass.service.Datum> getServiceName(String tableName) {
        List<org.yale.convene.BeenClass.service.Datum> serviceList = new ArrayList<>();
        SQLiteDatabase db = openDataBase();
        Logger.logV(TAG, tagStr + db.getPath());
        ...
        if (cursor.getCount() != 0 && cursor.moveToFirst()) {
            do {
                org.yale.convene.BeenClass.service.Datum datum = new org.yale.convene.BeenClass.service.Datum();
        ...
        try {
            ...
            if (cursor.getCount() != 0 && cursor.moveToFirst()) {
                do {
                    ...
                    JSONArray jsonArray = new JSONArray(addressString);
                    List<org.yale.convene.BeenClass.beneficiary.Address> addressList = new ArrayList<>();
                    for (int j = 0; j < jsonArray.length(); j++) {
                        Address address = new Address();
                        JSONObject jsonobject = jsonArray.getJSONObject(j);
                        address.setAddress1(jsonobject.getString(ADDRESS1_KEY));
     ...
     * @return
     */
    public List<org.yale.convene.BeenClass.facilities.Datum> getFacilityNameForFilter(String userEnterText, String btypeid, String modifiedDate) {
        List<org.yale.convene.BeenClass.facilities.Datum> facilityList = new ArrayList<>();
        List<Integer> servicesList = new ArrayList<>();
        SQLiteDatabase db = openDataBase();
        facilityList.clear();
     ...
     * @return
     */
    public List<org.yale.convene.BeenClass.facilities.Datum> getFilteredSurveyIdFacility(String query) {
        List<org.yale.convene.BeenClass.facilities.Datum> beneficiaryName = new ArrayList<>();
