...
import org.fwwb.convene.convenecode.BeenClass.Activitylist;
import org.fwwb.convene.convenecode.BeenClass.Levelid;
...
import org.fwwb.convene.convenecode.BeenClass.ResponsesData;
...
import org.fwwb.convene.convenecode.BeenClass.facilities.Facility;
import org.fwwb.convene.convenecode.BeenClass.facilitiesBeen.FacilitySubTypeBeen;
import org.fwwb.convene.convenecode.BeenClass.parentChild.Level1;
import org.fwwb.convene.convenecode.BeenClass.parentChild.LevelBeen;
import org.fwwb.convene.convenecode.BeenClass.parentChild.LevelFive;
import org.fwwb.convene.convenecode.BeenClass.parentChild.LevelFour;
import org.fwwb.convene.convenecode.BeenClass.parentChild.LevelOne;
import org.fwwb.convene.convenecode.BeenClass.parentChild.LevelSeven;
import org.fwwb.convene.convenecode.BeenClass.parentChild.LevelSix;
import org.fwwb.convene.convenecode.BeenClass.parentChild.LevelThree;
import org.fwwb.convene.convenecode.BeenClass.parentChild.LevelTwo;
import org.fwwb.convene.convenecode.BeenClass.parentChild.LinkagesList;
...
import org.fwwb.convene.convenecode.BeenClass.parentChild.SurveyListDetails;
...
public class ExternalDbOpenHelper extends SQLiteOpenHelper {
    ...
    public void updateSurveyList(SurveyListDetails surveyListDetails) {
        ...
        try {
            ...
            for (int i = 0; i < surveyListDetails.getSurveyDetails().size(); i++) {
                ...
                cv.put("qConfig", surveyListDetails.getSurveyDetails().get(i).getDisplayInTraining());
                ...
                String getRuleSet = new Gson().toJson(surveyListDetails.getSurveyDetails().get(i).getRuleEngine());
                cv.put("rule_engine", getRuleSet);
                cv.put("category_id", surveyListDetails.getSurveyDetails().get(i).getCategoryId());

                String getActivityruleSet = new Gson().toJson(surveyListDetails.getSurveyDetails().get(i).getActivityRuleSets());
                cv.put("activity_ruleset", getActivityruleSet);
                Log.v(TAG, "activity_ruleset: " + getActivityruleSet);
                ...
                cv.put("survey_display", surveyListDetails.getSurveyDetails().get(i).getDisplayInApp());
                cv.put("is_training_survey", surveyListDetails.getSurveyDetails().get(i).getIsTrainingSurvey());
        ...
        try {
            if (!linkages.isEmpty()) {
                ...
                for (int k = 0; k < linkages.size(); k++) {
                    ...
                    getformTypeIds.add(String.valueOf(form_type_id).trim());
                }
            }
     ...
     * @return
     */
    public List<org.fwwb.convene.convenecode.BeenClass.facilities.Datum> getFacilityDetailsFromDB(String btype, int filterOrder, String lastModifiedDate) {
        List<org.fwwb.convene.convenecode.BeenClass.facilities.Datum> facilityList = new ArrayList<>();
        ...
        try {
            ...
            if (cursor.getCount() != 0 && cursor.moveToFirst()) {
                do {
                    org.fwwb.convene.convenecode.BeenClass.facilities.Datum datum = new org.fwwb.convene.convenecode.BeenClass.facilities.Datum();
    ...
    private void setServicesToBean(String service, org.fwwb.convene.convenecode.BeenClass.facilities.Datum datum, List<Integer> servicesList) {
        String servicesSplitedString = service.replaceAll("\\[", "").replaceAll("\\]", "");
        if (!servicesSplitedString.isEmpty()) {
            String[] serviceList = servicesSplitedString.split(",");
            servicesList = new ArrayList<>();
     ...
     * @return
     */
    public List<org.fwwb.convene.convenecode.BeenClass.service.Datum> getServiceName(String tableName) {
        ...
        if (cursor.getCount() != 0 && cursor.moveToFirst()) {
            do {
                org.fwwb.convene.convenecode.BeenClass.service.Datum datum = new org.fwwb.convene.convenecode.BeenClass.service.Datum();
        ...
        try {
            ...
            if (cursor.getCount() != 0 && cursor.moveToFirst()) {
                do {
                    ...
                    JSONArray jsonArray = new JSONArray(addressString);
                    List<org.fwwb.convene.convenecode.BeenClass.beneficiary.Address> addressList = new ArrayList<>();
                    for (int j = 0; j < jsonArray.length(); j++) {
                        Address address = new Address();
                        JSONObject jsonobject = jsonArray.getJSONObject(j);
                        address.setAddress1(jsonobject.getString(ADDRESS1_KEY));
     ...
     * @return
     */
    public List<org.fwwb.convene.convenecode.BeenClass.facilities.Datum> getFacilityNameForFilter(String userEnterText, String btypeid, String modifiedDate) {
        List<org.fwwb.convene.convenecode.BeenClass.facilities.Datum> facilityList = new ArrayList<>();
        List<Integer> servicesList = new ArrayList<>();
        SQLiteDatabase db = openDataBase();
        facilityList.clear();
     ...
     * @return
     */
    public List<org.fwwb.convene.convenecode.BeenClass.facilities.Datum> getFilteredSurveyIdFacility(String query) {
        List<org.fwwb.convene.convenecode.BeenClass.facilities.Datum> beneficiaryName = new ArrayList<>();
    ...
    public HashMap<String, List<Datum>> getSurveyHeading() {
        ...
        try {
            SQLiteDatabase db = openDataBase();
            String query = "select category_name,category_id from Surveys where survey_display!=0 group by category_id";
...
//            String query = "select category_name,category_id from Surveys WHERE category_id!=0 group by category_id";
            String query = "select category_name,category_id from Surveys WHERE category_id!=0 and survey_display!=0 group by category_id";
            Cursor cursor = db.rawQuery(query, null);
            Logger.logD(TAG, "" + query);
            if (cursor != null && cursor.getCount() != 0 && cursor.moveToFirst()) {
                do {
    ...
    private List<Datum> getAllChiildReleated(String getCategoryID) {
        ...
        try {
            SQLiteDatabase db = openDataBase();
            String query = "select * from Surveys where category_id=" + getCategoryID + " and survey_display!=0 order by order_levels";
            Cursor cursor = db.rawQuery(query, null);
            Logger.logD(TAG, "" + query);
            if (cursor != null && cursor.getCount() != 0 && cursor.moveToFirst()) {
                do {
    ...
    public List<QuestionAnswer> getLinkageHeadings(String surveysId, ExternalDbOpenHelper dbHandler) {
        ...
        try {
            String selectQuery = "Select * from SurveyLinkage INNER JOIN Surveys on Surveys.surveyId =SurveyLinkage.form_type_id where SurveyLinkage.form_type_id IN (" + surveysId + ") and Surveys.survey_display=2 group by name";
    ...
    public String getSurveyRuleSet(int surveysId, ExternalDbOpenHelper dbhelper) {
        String ruleEngine = "";
        try {
            String pendingSurveyQuery = "select activity_ruleset from Surveys where Surveys.surveyId=" + surveysId;
            SQLiteDatabase db = dbhelper.getWritableDatabase();
            Cursor cursor = db.rawQuery(pendingSurveyQuery, null);
            if (cursor.getCount() != 0 && cursor.moveToFirst()) {
                do {
                    ruleEngine = cursor.getString(cursor.getColumnIndex("activity_ruleset"));
                } while (cursor.moveToNext());
