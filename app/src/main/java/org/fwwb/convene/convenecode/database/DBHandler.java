...
import org.fwwb.convene.convenecode.BeenClass.Linkage;
import org.fwwb.convene.convenecode.BeenClass.PreviewQuestionAnswerSet;
...
public class DBHandler extends SQLiteOpenHelper {
    ...
    public void updateAddressRecordToSurveyTable(String surveyPrimaryKeyId, Map<String, Spinner> dynamicSpinnerHashMap) {
        int getInsertedresult = 0;
        Set<String> getLocationMapKey= dynamicSpinnerHashMap.keySet();

        for (String s : getLocationMapKey) {
            LevelBeen getLevel1Id= new LevelBeen();
            switch (s){
                case "level1":
                    Spinner getLevel1Spinner = dynamicSpinnerHashMap.get("level1");
                    getLevel1Id = (LevelBeen) getLevel1Spinner.getSelectedItem();
                    break;
                case "level2":
                    Spinner getLevel2Spinner = dynamicSpinnerHashMap.get("level2");
                    getLevel1Id = (LevelBeen) getLevel2Spinner.getSelectedItem();
                    break;
                case "level3":
                    Spinner getLevel3Spinner = dynamicSpinnerHashMap.get("level3");
                    getLevel1Id = (LevelBeen) getLevel3Spinner.getSelectedItem();
                    break;
                case "level4":
                    Spinner getLevel4Spinner = dynamicSpinnerHashMap.get("level4");
                    getLevel1Id = (LevelBeen) getLevel4Spinner.getSelectedItem();
                    break;
                case "level5":
                    Spinner getLevel5Spinner = dynamicSpinnerHashMap.get("level5");
                    getLevel1Id = (LevelBeen) getLevel5Spinner.getSelectedItem();
                    break;
                case "level6":
                    Spinner getLevel6Spinner = dynamicSpinnerHashMap.get("level6");
                    getLevel1Id = (LevelBeen) getLevel6Spinner.getSelectedItem();
                    Logger.logD("Print Level Result",getLevel1Id.getName());
                    break;
                case "level7":
                    Spinner getLevel7Spinner = dynamicSpinnerHashMap.get("level7");
                    getLevel1Id = (LevelBeen) getLevel7Spinner.getSelectedItem();
                    break;
            }
            try {
                SQLiteDatabase sqLiteDatabase = this.getWritableDatabase(DATABASESECRETKEY);
                ...
                LevelBeen  clusterBean = (LevelBeen) getLevel7Spinner.getSelectedItem();
                values.put("cluster_name", clusterBean.getName());
                values.put("cluster_code", clusterBean.getId());
    ...
    public ArrayList<childLink> getChildDetailsFromBeneficiaryLinkage(int surveysId, String surveyPrimaryKeyId) {
    ...
    public String getResponseChoise(String surveyPrimaryKeyId) {
        ...
        try {
            if (cursor.getCount() != 0 && cursor.moveToFirst()) {
                do {
                  int ans_code = cursor.getInt(cursor.getColumnIndex("ans_code"));
                  if (ans_code!=0){
                      getResponseText= String.valueOf(ans_code);
                  }
                } while (cursor.moveToNext());
            }
            cursor.close();
