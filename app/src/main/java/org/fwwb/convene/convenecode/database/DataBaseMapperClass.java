...
import org.fwwb.convene.convenecode.beansClassSetQuestion.Option;
import org.fwwb.convene.convenecode.beansClassSetQuestion.RuleEngine;
...
public class DataBaseMapperClass {
    ...
    public static HashMap<String,AnswersPage> getUserAnsweredResponseFromDB(int questionNumber, net.sqlcipher.database.SQLiteDatabase db, String surveyPrimaryKeyId, RestUrl restUrl) {
        ...
        try {
           ...
           Logger.logD("Response query-->",responseQuery);
            ...
            if (questionCursor.moveToFirst()) {
                do {
                    ...
                    String otherOptions = "";
                    try {
                        otherOptions = questionCursor.getString(questionCursor.getColumnIndex("sub_questionId"));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    if (!otherOptions.isEmpty() && !otherOptions.equalsIgnoreCase("0")){
                        isOtherOption=1;
                    }
                    AnswersPage answerResponse= new AnswersPage(ansCode,answer,0,otherOptions,questionCursor.getInt(questionCursor.getColumnIndex(ansCodeStr)),0,0,"","",isOtherOption);
    ...
    public static List<Page> getQuestionOnBlocks(int qcode, SQLiteDatabase database, int survey_id, int language_id, List<String> mainQlist) {
        ...
        if (gridOrNot){
            if (language_id==1)
                QuestionQuery="SELECT DISTINCT Question.help_text,Question.question_Ucode,Question.question_json, Question.id, Question.block_id, Question.question_code ,Question.question_id, Question.answer_type,Question.mandatory, Question.question_text, Question.validation from Question , Options  where Question.id="+qcode+" and Question.active = 2 and Question.survey_id= " + survey_id;
            else
            ...
            if (language_id==1)
                QuestionQuery="SELECT DISTINCT  Question.id,Question.question_Ucode,Question.question_json,Question.validation,Question.location_levels,Question.question_id, Question.block_id, Question.help_text, Question.question_code, Question.answer_type,  Question.mandatory, Question.question_text, Question.validation from Question , Options  where Question.id="+qcode+" and Question.id=Options.question_pid and Question.survey_id =" + survey_id;
            else if(language_id!=1 && checkLanguage)
                QuestionQuery = "SELECT DISTINCT  Question.id,Question.question_Ucode,Question.question_json,Question.location_levels,Question.question_id, Question.block_id, Question.help_text, Question.question_code, Question.answer_type,  Question.mandatory, LanguageQuestion.question_text, Question.validation from Question ,LanguageQuestion  where Question.id=" + qcode + " and  LanguageQuestion.question_pid=Question.id and LanguageQuestion.language_id=" + language_id + " and  Question.survey_id= " + survey_id;
            else
        ...
        try {
            ...
            if(cursor.getCount()>0 && cursor.moveToFirst()){
                do{
                    ...
                    String questionUcode = String.valueOf(cursor.getInt(cursor.getColumnIndex("question_Ucode")));
                   ...
                   //Modify by guru start
                    QuestionJson questionJson1 = null;

                    try {
                        String questionJson = cursor.getString(cursor.getColumnIndex("question_json"));

                        if (questionJson != null && !questionJson.isEmpty()) {
                            Gson gson = new Gson();
                            questionJson1 = gson.fromJson(questionJson, QuestionJson.class);
                        }
                    }catch (Exception e)
                    {
                        Logger.logE(TAG,"getQuestionOnBlocks",e);
                    }
                    Page page=new Page();
                    ...
                    page.setQuestionJson(questionJson1);
                    page.setQuestionUcode(questionUcode);
                    pages.add(page);
                    Count=Count+1;
                }while (cursor.moveToNext());
            }
...
//            storeAnswer.add(answersPageDefault);
    ...
    public static HashMap<String,List<AnswersPage>> getAnswerFromDBnew(int questionNumber, SQLiteDatabase database, RestUrl restUrl, int languageId) {
        ...
        Cursor cursor=null;
        if (languageId!=1)
            QuestionQuery="SELECT a.id,l.option_text,a.other_choice from Options a, Question q, LanguageOptions l  where a.question_pid=q.id and q.id="+questionNumber+" and l.option_pid=a.id and l.language_id="+languageId+ORDER_BY_OPTION_ORDER;
        else
            QuestionQuery="SELECT a.id,option_text,a.other_choice from Options a, Question q where a.question_pid=q.id and q.id="+questionNumber+ORDER_BY_OPTION_ORDER;
        try {
            cursor=database.rawQuery(QuestionQuery,null);
            Logger.logD(TAG,questionQueryStr + QuestionQuery);
            if(cursor.moveToFirst()){
                do{
                    ...
                    int otherChoice=cursor.getInt(cursor.getColumnIndex("other_choice"));
                    AnswersPage answersPage= new AnswersPage(String.valueOf(pid),answer,0,"",0,0,0,"","",otherChoice);
                    storeAnswer.add(answersPage);
    ...
    public static List<LevelBeen> getBenificiaryParentDetails(net.sqlcipher.database.SQLiteDatabase db, String selectQuery) {
    ...
    public static List<LevelBeen> getSurveyDetails(ExternalDbOpenHelper helper, Integer categoryId) {
       List<LevelBeen> getBeneficiaryParentList= new ArrayList<>();
        String selectQuery = "SELECT Surveys.surveyId, Surveys.surveyName FROM Surveys where qConfig=2 and category_id = "+categoryId;
        LevelBeen levelBeenDefault= new LevelBeen();
        levelBeenDefault.setUuid("0");
        levelBeenDefault.setName("Select");
        getBeneficiaryParentList.add(levelBeenDefault);
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                String name = cursor.getString(cursor.getColumnIndex("surveyName"));
                LevelBeen levelBeen= new LevelBeen();
                levelBeen.setUuid(surveyid);
                levelBeen.setName(name);
                getBeneficiaryParentList.add(levelBeen);
    ...
    public static String isLocationExist(String surveyPrimaryKeyId, net.sqlcipher.database.SQLiteDatabase db,String getLocationLevel) {
        String getResponseText="";
        String selectQuery = "select "+getLocationLevel+" from survey where uuid='"+surveyPrimaryKeyId+"'";

        Cursor cursor = db.rawQuery(selectQuery, null);
        if (cursor != null && cursor.moveToFirst()) {
            do {
                getResponseText = cursor.getString(cursor.getColumnIndex(getLocationLevel));
            } while (cursor.moveToNext());
        }
        if (cursor != null)
            cursor.close();
    ...
    public static String getParentUUID(net.sqlcipher.database.SQLiteDatabase db, String surveyPrimaryKeyId) {
        String getUUID="";
        String selectQuery = "select beneficiary_ids from Survey where uuid='"+surveyPrimaryKeyId+"'";
        Cursor cursor = db.rawQuery(selectQuery, null);
        if (cursor != null && cursor.moveToFirst()) {
            do {
                getUUID = cursor.getString(cursor.getColumnIndex("beneficiary_ids"));
            } while (cursor.moveToNext());
        }
        cursor.close();
        return getUUID;
