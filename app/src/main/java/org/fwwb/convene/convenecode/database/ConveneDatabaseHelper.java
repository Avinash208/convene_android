...
import org.fwwb.convene.convenecode.BeenClass.beneficiaryList.Jsondata;
import org.fwwb.convene.convenecode.BeenClass.regionallanguage.GetLanguageAssessment;
import org.fwwb.convene.convenecode.BeenClass.regionallanguage.GetLanguageBlock;
...
import org.fwwb.convene.convenecode.BeenClass.regionallanguage.RegionalLanguage;
import org.fwwb.convene.convenecode.beansClassSetQuestion.BlockBeen;
import org.fwwb.convene.convenecode.beansClassSetQuestion.LanguageLabelsBeen;
import org.fwwb.convene.convenecode.beansClassSetQuestion.OptionsBeen;
import org.fwwb.convene.convenecode.beansClassSetQuestion.QuestionBeen;
import org.fwwb.convene.convenecode.beansClassSetQuestion.SkipMandatoryBeen;
        ...
        try {
            ...
            for (int i = 0; i < listOfObjects.getOptions().size(); i++) {
               ...
               // if (listOfObjects.getOptions().get(i).getId()==1773){
                    cv.put("other_choice", listOfObjects.getOptions().get(i).isOtherChoice());
    ...
    public void updateQuestions(QuestionBeen listOfObjects) {
        try{
            ...
            for(int i=0;i<listOfObjects.getQuestion().size();i++)
            {
                ...
                cv.put("display_as_name",String.valueOf(listOfObjects.getQuestion().get(i).getDisplayAsName()));
                ...
                String json;
                json = gson.toJson(listOfObjects.getQuestion().get(i).getQuestionJson());
                if (json != null) {
                    cv.put("question_json", json);
                } else {

                    cv.put("question_json", "");
                }
