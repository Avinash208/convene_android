...
import org.fwwb.convene.convenecode.BeenClass.ActivityRuleSet;
...
import org.fwwb.convene.convenecode.network.InsertResponseDump;
import org.fwwb.convene.convenecode.network.InsertTask;
import org.fwwb.convene.convenecode.network.SurveyGridInlineInterface.surveyQuestionGridInlineInterface;
import org.fwwb.convene.convenecode.network.SurveyGridInlineInterface.surveyQuestionPreviewInterface;
import org.fwwb.convene.convenecode.utils.CheckNetwork;
import org.fwwb.convene.convenecode.utils.CommonForAllClasses;
import org.fwwb.convene.convenecode.utils.Constants;
import org.fwwb.convene.convenecode.utils.ConstantsUtils;
import org.fwwb.convene.convenecode.utils.FileUtils;
import org.fwwb.convene.convenecode.utils.FontUtils;
import org.fwwb.convene.convenecode.utils.InternalStorageContentProvider;
...
import org.fwwb.convene.convenecode.utils.QuestionActivityUtils;
import org.fwwb.convene.convenecode.utils.RestUrl;
import org.fwwb.convene.convenecode.utils.RuleEngineUtils;
...
import org.fwwb.convene.convenecode.utils.multispinner.SingleSpinnerSearchFilter;
import org.fwwb.convene.convenecode.utils.multispinner.SpinnerListenerFilter;
...
import static org.fwwb.convene.convenecode.database.DataBaseMapperClass.getUserAnsweredResponseFromDB;
import static org.fwwb.convene.convenecode.database.DataBaseMapperClass.setAnswersForGrid;
...
import static org.fwwb.convene.convenecode.utils.Constants.gridSubQuestionMapDialog;
...
public class SurveyQuestionActivity extends BaseActivity implements View.OnClickListener, surveyQuestionPreviewInterface,
    ...
    private Map<Integer,DependencyObject> dependencyObjectMap = new HashMap<>();
    private Map<Integer,SingleSpinnerSearchFilter> singleSpinnerSearchFilterMap = new HashMap<>();
    private Map<Integer,LevelBeen> singleSpinnerAnswerMap = new HashMap<>();
    private int apiQCount = 0;
    private List<Integer> getBenificiaryQids = new ArrayList<>();
    ...
    private String selectSurveyQuery = "SELECT Response.ans_text, Survey.uuid FROM Response INNER JOIN Survey ON Response.survey_id = Survey.uuid where Response.q_code=";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        updateHeadingDetailsInBackGround(surveyPrimaryKeyId);
        ...
        if ("Yes".equals(surveyPreferences.getString(PreferenceConstants.RESUME_SURVEY, ""))) {
            ...
            if (!activityRuleSet.isEmpty()) {
                List<String> getRuleSetQuestion = applyRuleEnginIfExist(activityRuleSet);
                if (!getRuleSetQuestion.isEmpty()) {
                    ModuleToSetForm(count, pageSetCount, getRuleSetQuestion);
                }else{
                    ModuleToSetForm(count, pageSetCount, mainQList);
                }
            }else {
    ...
    private void updateHeadingDetailsInBackGround(String surveyPrimaryKeyId) {
    ...
    private void ModuleToSetForm(int count, int pageSetCount, List<String> tempQidsList) {
        ...
        try {
            ...
            // UpdatedCOde to Add Not assigned Qids to the Flow .
    ...
    private List<String> applyRuleEnginIfExist(String activityRuleSet) {
        ...
        try {
            ...
            for (int j = 0; j < jsonArray.length(); j++) {
                if (validateRuleset.contains(false))
                    return tempQid;
    ...
    private boolean validateOperator(String getOperator, String ruleSetvalue, String getOptionCode) {
        switch (getOperator) {
            case ">":
    ...
    private void inLineGridDisplay(Page page) {
        ...
        dynamicInlineAdd.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                SupportClass.addOREditFlag="add";
                    ...
                    if (spiltButtonTag[1].equals("ADD")) {
                        ...
                        if (GridResponseHashMap.size() > 0) {
                            ...
                            if (getAnsweredResponse.size() > 0) {
                                ...
                                SupportClass supportClass= new SupportClass();
                                SupportClass.addOREditFlag="edit";
    ...
    private void beneficiaryParentDisplay(Page page, String questionFont, String answerFont, int questionCode) {
        ...
        SingleSpinnerSearchFilter spinnerSearch;
        spinnerSearch = new SingleSpinnerSearchFilter(this);
        singleSpinnerContainer.addView(spinnerSearch);
        dynamicQuestionSet.addView(child);
        singleSpinnerSearchFilterMap.put(page.getQuestionNumber(),spinnerSearch);
        List<LevelBeen> list;
        HashMap<String, AnswersPage> getDataAnswer;   // getting if already answered question

        //Modify by guru start
        String selectQuery = selectSurveyQuery+page.getPartnerId();
        if (page.getQuestionJson() == null)
        {

            list = DataBaseMapperClass.getBenificiaryParentDetails(db, selectQuery);
            getDataAnswer = getUserAnsweredResponseFromDB(page.getQuestionNumber(), db, surveyPrimaryKeyId, restUrl);
        }
        else
        {

            if ("survey".equalsIgnoreCase(page.getQuestionJson().getDependencyDatatype()))
            {
                list = DataBaseMapperClass.getSurveyDetails(dbhelper, /*page.getQuestionJson().getReferenceId()*/1);
                getDataAnswer = getUserAnsweredResponseFromDB(page.getQuestionNumber(), db, surveyPrimaryKeyId, restUrl);
            }
            else if ("Response".equalsIgnoreCase(page.getQuestionJson().getDependencyDatatype())) {
...
//                            getParentsBeneficiaryName = finalList.get(j).getName();
        ...
        if (dependencyObjectMap.get(questionCode) != null)
        {
            setDependencyData(dependencyObjectMap.get(questionCode));
        }
    }
    ...
    //Modify by guru
    private void setDependencyData(DependencyObject dependencyObject) {

        switch (dependencyObject.getDependingDatatype())
        {
            case "response":
                setResponseValueToDependencyquestion(dependencyObject);
                break;
            case "survey":
                setSurveyValueToDependencyquestion(dependencyObject);
                break;
            default:
                setResponseValueToDependencyquestion(dependencyObject);
                break;
    ...
    private void setSurveyValueToDependencyquestion(DependencyObject dependencyObject) {

        if (singleSpinnerSearchFilterMap.get(dependencyObject.getQuestionId()) == null)
            return;



        List<LevelBeen> levelBeenList;
        levelBeenList = DataBaseMapperClass.getSurveyDetails(dbhelper, Integer.valueOf(dependencyObject.getValue()));
        ...
        int getIndex = 0;
        if (!getDataAnswer.isEmpty()) {
            for (int p = 0; p < levelBeenList.size(); p++) {
                AnswersPage answersPage = getDataAnswer.get(String.valueOf(dependencyObject.getDependencyQid()));
                if (answersPage.getAnswer().equals(levelBeenList.get(p).getUuid()))
                    getIndex = p;
            }
        }


        List<LevelBeen> finalList = levelBeenList;
        SingleSpinnerSearchFilter spinnerSearch = singleSpinnerSearchFilterMap.get(dependencyObject.getQuestionId());
        spinnerSearch.setFilterItems(levelBeenList, getIndex, new SpinnerListenerFilter() {
                ...
                try {
                    for (int j = 0; j < items.size(); j++) {
                        if (items.get(j).isSelected()) {
                            ...
                            //Modify by guru start
                            getBenificiaryQids.add(dependencyObject.getDependencyQid());
                            singleSpinnerAnswerMap.put(dependencyObject.getDependencyQid(),finalList.get(j));
                            //Modify by guru end
    ...
    private void setResponseValueToDependencyquestion(DependencyObject dependencyObject) {
        ...
        if (dependencyObject.getQuestionId()== 0) {
            ...
            String selectQuery = selectSurveyQuery+questionId;
            levelBeenList = DataBaseMapperClass.getBenificiaryParentDetails(db, selectQuery);
        }
        else
        {
            questionId = String.valueOf(dependencyObject.getQuestionId());
            String selectQuery = "SELECT Response.ans_text, Survey.uuid FROM Response INNER JOIN Survey ON Response.survey_id = Survey.uuid where Survey.training_survey_id='"+dependencyObject.getValue()+"' and  Response.q_code="+questionId;
            levelBeenList = DataBaseMapperClass.getBenificiaryParentDetails(db, selectQuery);
        ...
        List<LevelBeen> finalList = levelBeenList;
        levelBeenList.get(getIndex).setSelected(true);
//        SingleSpinnerSearchFilter spinnerSearch = singleSpinnerSearchFilterMap.get(dependencyObject.getDependencyQid());

        singleSpinnerSearchFilterMap.get(dependencyObject.getDependencyQid()).setFilterItems(levelBeenList, 0, new SpinnerListenerFilter() {
    ...
    private void addressWidgetDisplay(Page page, String questionFont, String answerFont, int questionCode) {
        ...
        String[] orderLabels;
        String[] orderLeves;
        String levels = dbhelper.getOrderLevels(surveysId);
        String labels = dbhelper.getOrderlabels(surveysId);
        orderLabels = labels.split(",");
        orderLeves = levels.split(",");
       /* orderLabels={"Grama panchayath","Village","Hamlet"};
        orderLeves={"level5","level6","level7"};*/
        try {
            for (int i = 0; i < orderLeves.length; i++) {
                ...
                dynamicSpinnerHashMap.put(orderLeves[i], dynamicSpinner);
                dynamicQuestionSet.addView(child);
    ...
    private void setValuesToSpinner(String orderLeve, Spinner dynamicSpinner) {
        ...
        if (!getLevelsrecords.isEmpty()) {
            ...
            String isLocationExist = DataBaseMapperClass.isLocationExist(surveyPrimaryKeyId, db, orderLeve);
            if (!isLocationExist.isEmpty()) {
                for (int w = 0; w < getLevelsrecords.size(); w++) {
                    if (getLevelsrecords.get(w).getId() == Integer.parseInt(isLocationExist)) {
                        dynamicSpinner.setSelection(w);
                    }
                }
            }
        }
        ...
        try {
            ...
            for (int i = selectedLevel; i < storeAllDynamicSpinner.size() - 1; i++) {
                ...
                if (!stateList.isEmpty()) {
                    ...
                    String isLocationExist = DataBaseMapperClass.isLocationExist(surveyPrimaryKeyId, db, String.valueOf("level" + getLevel));
                    if (!isLocationExist.isEmpty()) {
                        for (int w = 0; w < stateList.size(); w++) {
                            if (stateList.get(w).getId() == Integer.parseInt(isLocationExist)) {
                                nextSpinner.setSelection(w);
                            }
                        }
                    }
                } else {
    ...
    private void updateAllLocationToDatabase() {
        Logger.logD("DynamicSpinner" + dynamicSpinnerHashMap.size(), "");
    }
    ...
    public void radioGroupQuestionDisplay(Page displayQuestionModel, String questionFont, String answerFont, int questionCode) {
        ...
        LinearLayout otherTextContainer = child.findViewById(R.id.otherparentlayout);
        question.setTextSize(Integer.valueOf(questionFont));
        question.setFocusable(true);
        ...
        for (int j = 0; j < answerEditList.size(); j++) {
            ...
            radioButtons[j].setTag(answerValues.get(String.valueOf(questionCode)).get(j).getOther_choice());
            ...
            try {
                if (answerValues.get(String.valueOf(questionCode)).get(j).getAnswerCode().equals(getradioQuestionAnswer.get(String.valueOf(questionCode)).getAnswerCode())
                        && answerValues.get(String.valueOf(questionCode)).get(j).getOther_choice() == 1) {
                    otherTextContainer.setVisibility(View.VISIBLE);
                    EditText otherOptionEditText = (EditText) child.findViewById(R.id.otherentrytext);
                    otherOptionEditText.setText(getradioQuestionAnswer.get(String.valueOf(questionCode)).getValidation());
                    otherOptionEditTextMap.put(String.valueOf(displayQuestionModel.getQuestionNumber()), otherOptionEditText);
                } else {
                    otherTextContainer.setVisibility(View.GONE);
                }
            } catch (Exception e) {
                Logger.logE("Exception in radio", "location", e);
            }
    ...
    public void dropDownQuestionDisplay(Page displayQuestionModel, String questionFont, String answerFont, int questionCode) {
        ...
        spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                if (spinnerAnswer.get(i).getOther_choice() == 1) {
                    otherTextContainer.setVisibility(View.VISIBLE);
                    EditText otherOptionEditText = (EditText) child.findViewById(R.id.otherentrytext);
                    otherOptionEditTextMap.put(String.valueOf(displayQuestionModel.getQuestionNumber()), otherOptionEditText);
        ...
        if (!getspinnerQuestionAnswer.isEmpty() && getspinnerQuestionAnswer.get(String.valueOf(questionCode)).getOther_choice() == 1) {
            otherTextContainer.setVisibility(View.VISIBLE);
            EditText otherOptionEditText = (EditText) child.findViewById(R.id.otherentrytext);
            otherOptionEditText.setText(getspinnerQuestionAnswer.get(String.valueOf(questionCode)).getValidation());
        }
            ...
            for (int i = 0; i < mCount  ; i++) {
                ...
                switch (Integer.parseInt(listQuestionType.get(i))) {
                    ...
                    case 10:
                        ...
                        int apiQId = getBenificiaryQids.get(apiQCount);
                        boolean checkMandatory = DBHandler.getMandatoryQuestion(String.valueOf(apiQId), surveyDatabase);
                        if (!checkMandatory ||(!singleSpinnerAnswerMap.get(getBenificiaryQids.get(apiQCount)).getUuid().equals("") && !singleSpinnerAnswerMap.get(getBenificiaryQids.get(apiQCount)).getName().contains("Select"))) {
                            errorTextView.setVisibility(View.GONE);
                            list.add(String.valueOf(true));
                            fillAIResponseToDB(getBenificiaryQids.get(apiQCount), singleSpinnerAnswerMap.get(getBenificiaryQids.get(apiQCount)).getUuid(), 10, singleSpinnerAnswerMap.get(getBenificiaryQids.get(apiQCount)).getName());
                            apiQCount++;
                        } else {
                            list.add(String.valueOf(false));
                            errorTextView.setText("Select option");
    ...
    private void fillAllLevelToDatabase() {
        try {
            surveyHandler.updateAddressRecordToSurveyTable(surveyPrimaryKeyId, dynamicSpinnerHashMap);
            Spinner getLevel7Spinner = dynamicSpinnerHashMap.get("level7");
            LevelBeen getLevel7Id = (LevelBeen) getLevel7Spinner.getSelectedItem();
    ...
    private boolean spinnerFunctionality(final int spinnerQuestionCode, final SQLiteDatabase database, final int qType) {
        ...
        if (checkMandatory) {
            ...
            if (getSpinnerBean.getOther_choice() == 1) {
                EditText editText = otherOptionEditTextMap.get(String.valueOf(spinnerQuestionCode));
                if (!editText.getText().toString().isEmpty()) {
                    fillResponseToDBNONwidget(spinnerQuestionCode, getSpinnerBean.getAnswer(), String.valueOf(getSpinnerBean.getId()), qType, editText.getText().toString()); // filling Question and Answer to Been and Table

                } else {
                    errorText.setVisibility(View.VISIBLE);
                    errorText.setText("Please enter optional");
                    errorText.startAnimation(animShake);
                    errorText.setFocusableInTouchMode(true);
                    spinnerBoolean = false;
                }
            ...
            fillResponseToDBNONwidget(spinnerQuestionCode, getSpinnerBean.getAnswer(), getAnswercode, qType,
                    ""); // filling Question and Answer to Been and Table
    ...
    private boolean radioFunctionality(final int questionCode, final SQLiteDatabase database, final int qType) {
        ...
        for (int i = 0; i < radioGroup[0].getChildCount(); i++) {
            ...
            if (button.isChecked()) {
                radioBoolean = true;
                Logger.logD("RadioGroup Other", button.getTag() + "");
                int getRadioOtherQuestionIds = (int) button.getTag();
                if (getRadioOtherQuestionIds == 1) {
                    EditText editText = otherOptionEditTextMap.get(String.valueOf(questionCode));
                    if (!editText.getText().toString().isEmpty()) {
                        getOtherOptions = editText.getText().toString().trim();

                    } else {
                        errorTextView.setVisibility(View.VISIBLE);
                        errorTextView.setText("Please enter optional");
                        errorTextView.startAnimation(animShake);
                        errorTextView.setFocusableInTouchMode(true);
                        return false;
                    }
                ...
                radioAnswerCode = getAnswercode;
                fillResponseToDBNONwidget(questionCode, checkedItemData, getAnswercode, qType, getOtherOptions); // filling Question and Answer to Been and Table
    ...
    private void fillResponseToDB(int questionCode, String answer, int qType) {
        ...
        hashMapAnswersEditText.put(String.valueOf(questionCode) + "_" + String.valueOf(qType), answersCollection);
    }
     ...
     * @param answerCode
     * @param qType
     * @param otherOption     this parameter is for other specify . send selected ids (here it will be saving other choise ids)
     */
    public void fillResponseToDBNONwidget(int questionCode, String answer, String answerCode, int qType, String otherOption) {
        ...
        int survey_ID = getSharedPreferences(MY_PREFS_NAME_SURVEY, MODE_PRIVATE).getInt(SURVEYID, 0);
        Response response = new Response(String.valueOf(questionCode), answer, answerCode, otherOption, questionCode, 0, String.valueOf(survey_ID), 0, Integer.parseInt(optionPID), QuestionActivityUtils.getQuestionType(qType));
